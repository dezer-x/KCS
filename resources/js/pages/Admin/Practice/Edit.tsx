...
interface AdminPracticeEditProps extends SharedData {
  server: PracticeServer;
  mapImages: string[];
}

export default function AdminPracticeEdit() {
  const { auth, server, mapImages } = usePage<AdminPracticeEditProps>().props;
  const audioRef = useRef<HTMLAudioElement>(null);
  const [selectedMapImage, setSelectedMapImage] = useState<string>(server.map_image || '');
  ...
  useEffect(() => {
    ...
    setSelectedMapImage(server.map_image || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('Sending PATCH request to:', `/admin/practice/${server.id}`);
    patch(`/admin/practice/${server.id}`, {
      onSuccess: () => {
